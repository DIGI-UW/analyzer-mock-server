...
def main():
    ...
    # Start unified API server in background thread if requested
    api_port = getattr(args, 'simulate_api_port', None) or getattr(args, 'api_port', None)
    if api_port:
        ...
        api_thread.start()
        logger.info("API server started on port %s (background)", api_port)
